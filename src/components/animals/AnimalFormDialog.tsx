...
export function AnimalFormDialog({ open, onOpenChange, editingAnimal, userCabaña, parentAnimals, onSuccess }: Props) {
  ...
  const DetailFields = ({ row, onChange }: { row: AnimalDraft; onChange: (patch: Partial<AnimalDraft>) => void }) => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 bg-muted/30 rounded-lg border">
      ...
      {row.sex === 'Macho' && <><div className="space-y-1"><Label>{t('animals:form.scrotalCircumference')}</Label><Input type="number" min="0" step="0.1" value={row.circunferencia_escrotal} onChange={e => onChange({ circunferencia_escrotal: e.target.value })} /></div><div className="space-y-1"><Label>{t('animals:form.castrated')}</Label>{renderSelect(row.is_castrated ? 'yes' : 'no', v => onChange({ is_castrated: v === 'yes' }), t('common:no'), [{ value: 'yes', label: t('common:yes') }, { value: 'no', label: t('common:no') }])}</div></>}
      {row.sex === 'Hembra' && <><div className="space-y-1"><Label>{t('animals:profile.pregnant')}</Label>{renderSelect(row.esta_preñada ? 'yes' : 'no', v => onChange({ esta_preñada: v === 'yes' }), t('common:no'), [{ value: 'yes', label: t('common:yes') }, { value: 'no', label: t('common:no') }])}</div><div className="space-y-1"><Label>{t('animals:profile.expectedCalving')}</Label><Input type="date" value={row.fecha_probable_parto} onChange={e => onChange({ fecha_probable_parto: e.target.value })} /></div></>}
      <div className="space-y-1"><Label>ADN</Label>{renderSelect(row.dna_verified ? 'yes' : 'no', v => onChange({ dna_verified: v === 'yes' }), t('common:no'), [{ value: 'yes', label: t('common:yes') }, { value: 'no', label: t('common:no') }])}</div>
