-- Drop and recreate get_mortality_reports function to fix ambiguous column reference
DROP FUNCTION IF EXISTS public.get_mortality_reports(uuid, date, date);

CREATE OR REPLACE FUNCTION public.get_mortality_reports(_user_id uuid, _date_from date DEFAULT NULL, _date_to date DEFAULT NULL)
RETURNS TABLE(
  id uuid,
  animal_id uuid,
