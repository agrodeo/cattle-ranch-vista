-- Fix remaining ambiguous column references in rpc_reproductive_detailed_metrics
...
BEGIN
  ...
  FROM animals a
  LEFT JOIN corrales c ON c.id = a.corral_id AND c.cabaña_id = (SELECT cabana_id FROM public.get_user_cabana_info(_user_id) LIMIT 1)
