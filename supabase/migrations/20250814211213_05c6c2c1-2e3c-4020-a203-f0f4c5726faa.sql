-- Fix the list_finance_reports function to properly handle date ranges and recurring data
CREATE OR REPLACE FUNCTION public.list_finance_reports(_user_id uuid, _from_date date DEFAULT NULL::date, _to_date date DEFAULT NULL::date)
...
BEGIN
  ...
  WHILE month_date <= range_end_date LOOP
    ...
    WHERE fr.cabaña_id = cab_id
      ...
      AND (fr.start_date <= month_date OR fr.start_date IS NULL)
