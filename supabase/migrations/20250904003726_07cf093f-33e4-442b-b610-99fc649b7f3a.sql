-- Update the corral KPIs function to calculate pregnancy rate correctly using current status
...
BEGIN
  ...
  WITH corral_animals AS (
    SELECT 
      ...
      -- Count reproductive females (hembras >= 15 months old)
      COUNT(CASE 
        WHEN a.sex = 'Hembra' 
  ...
  SELECT 
    ...
    -- Use reproductive females as denominator for pregnancy rate calculation
    CASE 
