-- Fix rpc_report_production_animals to handle null filters correctly
...
BEGIN
  ...
  weight_data AS (
    SELECT 
      ...
      -- Count weighings from weight history
  ...
  adg_calculations AS (
    SELECT 
      wd.*,
      -- ADG recent 90 days (use ganancia_diaria_kg from animals table)
      COALESCE(wd.ganancia_diaria_kg, 0) as adg_recent_90d,
      -- ADG season (same as ganancia_diaria_kg for simplicity)
      COALESCE(wd.ganancia_diaria_kg, 0) as adg_season,
      -- Yearling weight estimation
      CASE 
