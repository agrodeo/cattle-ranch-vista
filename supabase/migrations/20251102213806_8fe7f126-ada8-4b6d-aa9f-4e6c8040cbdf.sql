-- Fix rpc_corral_complete_kpis function - cast COUNT results to bigint
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE 0 
        END,
        COALESCE(SUM(fm.monto), 0),
        COUNT(DISTINCT CASE 
            WHEN e.tipo IN ('tratamiento_medico', 'diagnostico') 
            AND e.fecha >= CURRENT_DATE - INTERVAL '30 days'
            THEN a.id 
        END)::bigint,
        COUNT(DISTINCT CASE WHEN NOT COALESCE(avs.is_compliant, false) THEN a.id END)::bigint,
        COUNT(DISTINCT CASE 
            WHEN a.sex = 'Hembra' 
