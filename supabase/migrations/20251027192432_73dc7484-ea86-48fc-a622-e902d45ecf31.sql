-- Fix vaccination KPI calculation to check compliance with configured requirements
...
BEGIN
  ...
  -- Count mandatory vaccination requirements for this cabaña
  SELECT COUNT(*) INTO mandatory_requirements_count
  ...
  vaccination_stats AS (
    SELECT 
      ...
      -- Count animals that are compliant with ALL mandatory requirements
      COUNT(DISTINCT CASE 
        WHEN mandatory_requirements_count = 0 THEN 
          -- If no requirements configured, count animals with any recent vaccination
          CASE WHEN av.animal_id IS NOT NULL AND av.date >= CURRENT_DATE - INTERVAL '365 days' 
          THEN a.id END
        ELSE
          -- If requirements exist, check if animal meets all mandatory requirements
          CASE WHEN (
