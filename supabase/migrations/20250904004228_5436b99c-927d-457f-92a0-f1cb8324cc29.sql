-- Fix the corral KPIs function to handle age calculation correctly and show pregnant animals properly
...
BEGIN
  ...
  WITH corral_animals AS (
    SELECT 
      ...
      -- Count reproductive females (hembras >= 15 months old or without birth_date)
      COUNT(CASE 
        WHEN a.sex = 'Hembra' 
        AND (
          a.birth_date IS NULL 
          OR a.birth_date <= CURRENT_DATE  -- Only valid birth dates
          AND EXTRACT(MONTH FROM AGE(CURRENT_DATE, a.birth_date)) >= 15
        )
        THEN 1 
      END) as reproductive_females,
      -- Count pregnant females using current pregnancy status
      COUNT(CASE WHEN a.sex = 'Hembra' AND a.esta_preñada = true THEN 1 END) as pregnant_count,
  ...
  SELECT 
    ...
    -- Use ALL females as denominator if no reproductive females calculated correctly
    CASE 
