-- Update get_finance_summary to include recurring finances
...
BEGIN
  ...
  start_date := COALESCE(_from_date, '1900-01-01'::date);
  end_date := COALESCE(_to_date, CURRENT_DATE);
  
  -- Calculate months in range for recurring calculations
  months_in_range := EXTRACT(MONTH FROM AGE(end_date, start_date))::integer + 1;
  ...
  SELECT COALESCE(SUM(
    CASE 
      ...
      WHEN fr.frequency = 'daily' THEN fr.amount * (end_date - start_date + 1)
      ELSE 0
    END
  ...
  WHERE fr.cabaña_id = cab_id
    ...
    AND (fr.start_date <= end_date)
    AND (fr.end_date IS NULL OR fr.end_date >= start_date);
...
-- Update list_finance_reports to include recurring finances
