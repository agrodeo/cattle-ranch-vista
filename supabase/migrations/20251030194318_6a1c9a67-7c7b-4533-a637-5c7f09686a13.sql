-- Fix vaccination compliance calculation to properly handle sex-specific requirements
...
BEGIN
  ...
  vaccination_stats AS (
    SELECT 
      ...
      -- Count animals that are compliant with their applicable mandatory requirements
      COUNT(DISTINCT CASE 
        WHEN (
          -- Check if animal meets all mandatory requirements applicable to their sex
          SELECT COUNT(DISTINCT req.id)
          FROM public.cabaña_vaccination_requirements req
          WHERE req.cabaña_id = cabana_uuid
            AND req.is_active = true
            AND req.is_mandatory = true
            AND (req.sex_restriction IS NULL OR req.sex_restriction = a.sex)
            AND EXISTS (
              SELECT 1 
              FROM public.animal_vaccines av_inner
              WHERE av_inner.animal_id = a.id
                AND av_inner.requirement_id = req.id
                AND (av_inner.next_due IS NULL OR av_inner.next_due > CURRENT_DATE)
            )
        ) >= (
          -- Count total mandatory requirements applicable to this animal's sex
          SELECT COUNT(*)
          FROM public.cabaña_vaccination_requirements req2
          WHERE req2.cabaña_id = cabana_uuid
            AND req2.is_active = true
            AND req2.is_mandatory = true
            AND (req2.sex_restriction IS NULL OR req2.sex_restriction = a.sex)
        )
        AND (
          -- Ensure at least one requirement exists for this animal's sex
          SELECT COUNT(*)
          FROM public.cabaña_vaccination_requirements req3
          WHERE req3.cabaña_id = cabana_uuid
            AND req3.is_active = true
            AND req3.is_mandatory = true
            AND (req3.sex_restriction IS NULL OR req3.sex_restriction = a.sex)
        ) > 0
        THEN a.id 
