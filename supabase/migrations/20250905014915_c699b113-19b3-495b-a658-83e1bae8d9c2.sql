-- Fix pregnancy percentage calculation in rpc_reproductive_detailed_metrics function
...
BEGIN
  ...
  reproductive_calculations AS (
    SELECT 
      ...
      -- Calculate reproductive years (minimum 1 if animal is reproductive age)
      GREATEST(1, 
        CASE 
          WHEN ef.age_in_months IS NOT NULL AND ef.age_in_months >= 18 
          THEN CEIL((ef.age_in_months - 18)::NUMERIC / 12)
          ELSE 1 
        END
  ...
  final_calculations AS (
    SELECT 
      ...
      -- FIXED: Calculate individual pregnancy rate without the incorrect /10.0 division
      -- Cap at 100% for realistic percentages
      LEAST(100.0, 
        CASE 
