-- Create RPC function for temporal production analysis
...
AS $function$
DECLARE
  date_from_filter date := COALESCE(_date_from, CURRENT_DATE - INTERVAL '5 years');
...
BEGIN
  ...
  weight_data AS (
    ...
    WHERE awh.cabaña_id = _cabana_id
      ...
      AND (_tipo_pesaje = 'all' OR awh.tipo_pesaje = _tipo_pesaje)
  ),
  period_stats AS (
    SELECT 
      wd.periodo_text,
      wd.year_num,
      wd.periodo_order,
      ROUND(AVG(CASE WHEN wd.tipo_pesaje = 'nacimiento' THEN wd.peso_kg END), 2) as peso_nacimiento,
      ROUND(AVG(CASE WHEN wd.tipo_pesaje = 'destete' THEN wd.peso_kg END), 2) as peso_destete,
      ROUND(AVG(CASE WHEN wd.tipo_pesaje = 'final' THEN wd.peso_kg END), 2) as peso_final,
      ROUND(AVG(wd.ganancia_diaria), 3) as adg_avg,
      COUNT(DISTINCT wd.animal_id) as animal_count,
      COUNT(*) as weight_count,
      PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY wd.peso_kg) as p25,
      PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY wd.peso_kg) as p75
    FROM weight_data wd
  ...
  SELECT 
    ...
    CASE 
      WHEN wc.prev_peso IS NOT NULL AND wc.prev_peso > 0 THEN
        ROUND(((COALESCE(wc.peso_destete, wc.peso_final) - wc.prev_peso) / wc.prev_peso * 100), 2)
      ELSE NULL
    END as mejora_vs_anterior,
    ROUND(wc.p25, 2) as percentil_25,
    ROUND(wc.p75, 2) as percentil_75
