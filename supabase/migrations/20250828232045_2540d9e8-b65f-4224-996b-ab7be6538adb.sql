-- Fix subscription count by standardizing animal status values and updating function

-- First, standardize all animal status values to lowercase
UPDATE public.animals 
SET status = LOWER(status) 
WHERE status IS NOT NULL;

-- Update the get_subscription_status function to use case-insensitive comparison
...
BEGIN
  ...
  -- Count current animals (case-insensitive status check)
