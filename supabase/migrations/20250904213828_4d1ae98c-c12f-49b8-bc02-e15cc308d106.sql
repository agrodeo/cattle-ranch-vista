-- Fix reproductive metrics function to calculate real values instead of hardcoded zeros
...
BEGIN
  ...
  reproductive_stats AS (
    SELECT 
      ...
      -- Calculate realistic pregnancy rate
      CASE 
        WHEN COALESCE(ss.total_services, 0) > 0 THEN 
          ROUND(
            (GREATEST(
              COALESCE(ps.confirmed_pregnancies, 0),
              COALESCE(tp.tacto_pregnancies, 0),
              CASE WHEN ef.esta_preñada THEN 1 ELSE 0 END
            )::NUMERIC / ss.total_services::NUMERIC) * 100, 1
          )
        WHEN ef.esta_preñada THEN 100.0
        ELSE 0.0
      END as individual_pregnancy_rate,
      -- Calculate calving rate (calvings / pregnancies)
      CASE 
        WHEN GREATEST(
          COALESCE(ps.confirmed_pregnancies, 0),
          COALESCE(tp.tacto_pregnancies, 0),
