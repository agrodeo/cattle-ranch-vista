-- Fix round() function issue with double precision types
...
BEGIN
  ...
  period_stats AS (
    SELECT 
      ...
      PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY wd.peso_kg) as p25_raw,
      PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY wd.peso_kg) as p75_raw
    FROM weight_data wd
  ...
  SELECT 
    ...
    CASE 
      WHEN wc.prev_peso IS NOT NULL AND wc.prev_peso > 0 THEN
        ROUND(((COALESCE(wc.peso_destete, wc.peso_final) - wc.prev_peso) / wc.prev_peso * 100)::numeric, 2)
      ELSE NULL
