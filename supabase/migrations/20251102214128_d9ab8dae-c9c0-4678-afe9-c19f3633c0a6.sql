-- Fix type casting in rpc_corral_complete_kpis to ensure all bigint columns return bigint
...
BEGIN
    ...
    SELECT 
        c.id,
        c.name,
        COALESCE(COUNT(DISTINCT a.id), 0)::bigint,
        AVG(lw.weight)::numeric,
        COALESCE(AVG(
            CASE 
        ...
        CASE 
            ...
            ELSE 0 
        END::numeric,
        COALESCE(SUM(fm.monto), 0)::numeric,
        COALESCE(COUNT(DISTINCT CASE 
            WHEN e.tipo IN ('tratamiento_medico', 'diagnostico') 
            AND e.fecha >= CURRENT_DATE - INTERVAL '30 days'
            THEN a.id 
        END), 0)::bigint,
        COALESCE(COUNT(DISTINCT CASE WHEN NOT COALESCE(avs.is_compliant, false) THEN a.id END), 0)::bigint,
        COALESCE(COUNT(DISTINCT CASE 
            WHEN a.sex = 'Hembra' 
