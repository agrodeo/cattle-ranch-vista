-- Fix age calculation in rpc_report_reproductive_females function
DROP FUNCTION IF EXISTS public.rpc_report_reproductive_females(uuid, jsonb);

CREATE OR REPLACE FUNCTION public.rpc_report_reproductive_females(_user_id uuid, filters_json jsonb DEFAULT '{}'::jsonb)
 RETURNS TABLE(animal_id uuid, tag text, name text, category text, corral_id uuid, corral_name text, is_pregnant boolean, pregnancy_date date, expected_calving_date date, age_months integer, last_service_date date, services_count integer, pregnancy_checks_count integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  WITH eligible_females AS (
    SELECT a.id,
           ...
           CASE 
             ...
             -- FIXED age calculation: years * 12 + months
    ...
    WHERE a.cabaña_id = cabana_uuid
      ...
      AND (
        ...
        -- FIXED age calculation for filtering (at least 18 months old)
      ...
      ) -- At least 18 months old or no birth date
