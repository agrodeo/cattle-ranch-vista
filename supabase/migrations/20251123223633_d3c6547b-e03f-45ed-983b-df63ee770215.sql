-- Update rpc_corral_complete_kpis to include vaccination counts
...
RETURNS TABLE(
    ...
    total_vaccinations_given bigint,
    total_vaccinations_needed bigint,
...
BEGIN
    ...
    WITH corral_animals AS (
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN a.sex = 'Macho' THEN a.id END) as male_count,
            COUNT(DISTINCT CASE WHEN a.sex = 'Hembra' THEN a.id END) as female_count
        FROM corrales c
    ...
    pregnancy_data AS (
        SELECT 
            ...
            (COUNT(CASE WHEN a.esta_preñada = true THEN 1 END)::numeric / 
             NULLIF(COUNT(CASE WHEN a.sex = 'Hembra' THEN 1 END)::numeric, 0) * 100) as pregnancy_rate
        FROM animals a
        WHERE a.cabaña_id = _cabana_id
        AND a.status = 'activo'
        GROUP BY a.corral_id
    ...
    SELECT 
        ...
        0::bigint as consanguinity_risk_count, -- Calculated separately in frontend
