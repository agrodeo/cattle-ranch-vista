-- Add debug logging to the reproductive metrics function
...
BEGIN
  ...
  -- Debug: Count eligible females first
  ...
  WHERE a.cabaña_id = cabana_uuid
    ...
    AND (
      ...
      OR (a.birth_date <= CURRENT_DATE AND (EXTRACT(YEAR FROM AGE(CURRENT_DATE, a.birth_date)) * 12 + EXTRACT(MONTH FROM AGE(CURRENT_DATE, a.birth_date))) >= 15)
    )
  ...
  RAISE NOTICE 'Found % eligible females for cabaña %', debug_count, cabana_uuid;
  ...
  reproductive_calculations AS (
    SELECT 
      ...
      -- Cast reproductive years calculation to INTEGER (changed to 15 months threshold)
      GREATEST(1, 
        CASE 
          WHEN ef.age_in_months IS NOT NULL AND ef.age_in_months >= 15 
          THEN CEIL((ef.age_in_months - 15)::NUMERIC / 12)::INTEGER
          ELSE 1 
        END
      ) as reproductive_years,
      
      -- Count services for this animal - CAST TO INTEGER
      COALESCE(
        (
        ...
        0
      ) as lifetime_services,
      
      -- Count confirmed pregnancies - CAST TO INTEGER
      COALESCE(
        (
          SELECT COUNT(*)::INTEGER
        ...
        0
      ) as confirmed_pregnancies,
      
      -- Count pending pregnancies - CAST TO INTEGER
      COALESCE(
        (
          SELECT COUNT(*)::INTEGER
          FROM public.preñeces p
        ...
        0
      ) as pending_pregnancies,
      
      -- Count offspring (calvings) - CAST TO INTEGER
      COALESCE(
        (
          SELECT COUNT(*)::INTEGER
