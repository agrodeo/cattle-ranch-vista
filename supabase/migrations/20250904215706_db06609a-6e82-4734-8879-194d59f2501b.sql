-- Update pregnancy percentage calculation formula as specified
...
BEGIN
  ...
  pregnancies_stats AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE p.estado = 'pendiente') as pending_pregnancies,
  ...
  reproductive_stats AS (
    SELECT 
      ...
      -- Sum all pregnancy sources
      (
        COALESCE(ps.confirmed_pregnancies, 0) +
        COALESCE(tp.tacto_pregnancies, 0) +
        CASE WHEN ef.esta_preñada = true THEN 1 ELSE 0 END
      )::integer as lifetime_pregnancies,
      COALESCE(os.total_children, 0)::integer as lifetime_calvings,
      -- NEW FORMULA: (historic pregnancies + unborn pregnancies + offspring) / reproductive years / 10
      CASE 
        WHEN GREATEST(1, LEAST(
          ...
          ROUND(
            ((
              COALESCE(ps.confirmed_pregnancies, 0) +
              COALESCE(ps.pending_pregnancies, 0) +
              COALESCE(os.total_children, 0)
            )::NUMERIC / 
            GREATEST(1, LEAST(
              COALESCE(DATE_PART('year', AGE(CURRENT_DATE, COALESCE(ef.birth_date, CURRENT_DATE - INTERVAL '3 years')))::integer - 1, 1),
              CASE WHEN ef.birth_date IS NOT NULL THEN 
      ...
      -- Calculate realistic calving rate: MIN(100, calvings / total pregnancies * 100)
      LEAST(100.0,
        CASE 
          WHEN (
            COALESCE(ps.confirmed_pregnancies, 0) +
