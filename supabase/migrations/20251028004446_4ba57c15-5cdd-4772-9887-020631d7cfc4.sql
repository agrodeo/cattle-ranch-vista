-- Fix record_vaccination to properly match vaccine codes with requirements
...
BEGIN
  ...
  -- Get vaccine information to map code to name
  ...
  -- Find matching vaccination requirement using vaccine name
  -- Try to match by exact vaccine name first, then try common name variations
  SELECT * INTO requirement_record
  FROM public.cabaña_vaccination_requirements
  WHERE cabaña_id = user_cabana_id
    AND (
      vaccine_name ILIKE vaccine_record.name
      OR vaccine_name ILIKE '%' || CASE 
        WHEN _vaccine_code = 'fmd' THEN 'Aftosa'
        WHEN _vaccine_code = 'brucelosis' THEN 'Brucelosis'
        WHEN _vaccine_code = 'rabia' THEN 'Rabia'
        WHEN _vaccine_code = 'carbunclo' THEN 'Carbunclo'
        WHEN _vaccine_code = 'clostridiales' THEN 'Clostridiales'
        ELSE vaccine_record.name
      END || '%'
    )
    AND is_active = true
