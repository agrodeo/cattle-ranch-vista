-- Fix pregnancy rate calculation in rpc_corral_complete_kpis function
-- The issue is that EXTRACT(MONTH FROM AGE(...)) only returns the month component (0-11)
-- We need to calculate total months: years * 12 + months
...
BEGIN
  ...
  WITH corral_animals AS (
    SELECT 
      ...
      -- Count reproductive females (hembras >= 15 months old with valid birth dates)
      -- Fixed calculation: total months = years * 12 + months
      COUNT(CASE 
        WHEN a.sex = 'Hembra' 
        AND a.birth_date IS NOT NULL 
        AND a.birth_date <= CURRENT_DATE
        AND (EXTRACT(YEAR FROM AGE(CURRENT_DATE, a.birth_date)) * 12 + EXTRACT(MONTH FROM AGE(CURRENT_DATE, a.birth_date))) >= 15
        THEN 1 
