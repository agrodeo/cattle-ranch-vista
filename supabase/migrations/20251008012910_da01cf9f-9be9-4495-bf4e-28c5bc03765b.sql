-- Fix filter to include all animals with any weight recorded
...
BEGIN
  ...
  adg_birth_to_weaning AS (
    SELECT 
      ea.id as animal_id,
      CASE
        -- Birth to weaning using actual weighing dates from history
        ...
        THEN
          (SELECT ROUND(
            ...
            NULLIF((MAX(awh_destete.fecha) - COALESCE(ea.birth_date, MIN(awh_birth.fecha)))::numeric, 0),
            3
          )
          FROM animal_weight_history awh_destete
          LEFT JOIN animal_weight_history awh_birth ON awh_birth.animal_id = ea.id AND awh_birth.tipo_pesaje = 'nacimiento'
          WHERE awh_destete.animal_id = ea.id 
          AND awh_destete.tipo_pesaje = 'destete')
        
        -- Fallback: use fecha_destete if available
        WHEN ea.peso_nacimiento IS NOT NULL 
          AND ea.peso_destete IS NOT NULL 
          AND ea.birth_date IS NOT NULL 
          AND ea.fecha_destete IS NOT NULL
        THEN
          ROUND(
            (ea.peso_destete - ea.peso_nacimiento) / 
            NULLIF((ea.fecha_destete - ea.birth_date)::numeric, 0),
            3
          )
        
        ELSE NULL
      END as adg_birth_weaning_value
    FROM eligible_animals ea
  ),
  adg_season_calc AS (
    SELECT 
  ...
  combined_data AS (
    SELECT 
      wd.*,
      adg_bw.adg_birth_weaning_value,
    ...
    FROM weight_data wd
    LEFT JOIN adg_birth_to_weaning adg_bw ON wd.animal_id = adg_bw.animal_id
  ...
  SELECT 
    ...
    pd.last_weight_date,
    pd.adg_birth_weaning_value as adg_recent_90d,
  ...
  WHERE pd.last_weight IS NOT NULL 
     OR pd.weighs_count > 0
     OR pd.weight_birth IS NOT NULL
     OR pd.weight_weaning IS NOT NULL
     OR pd.weight_final IS NOT NULL
