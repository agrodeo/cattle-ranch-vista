  ...
  try {
    ...
    for (const passwordRecord of passwords) {
      try {
        ...
        const hashedPassword = await bcrypt.hash(passwordRecord.password_text, saltRounds)
        ...
        // Log security event with system user
        await supabaseAdmin.rpc('log_security_event', {
          _action: 'password_hashed_migration',
          _table_name: 'user_passwords',
          _record_id: passwordRecord.user_id,
          _details: { migrated_by: 'system_migration' }
        })
    ...
    console.error('Error in migrate-employee-passwords function:', error)
    return new Response(
